...
const Components = {
    // Inline Exercise Builder
    InlineExerciseBuilder: {
        ...
        setupEventListeners() {
            const exerciseInput = document.getElementById('exercise-search');
            ...
            if (exerciseInput) {
                ...
                exerciseInput.addEventListener('focus', () => {
                    this.showExerciseDropdown();
                });
                
                exerciseInput.addEventListener('blur', (e) => {
                    // Delay hiding to allow option selection
                    setTimeout(() => {
                        if (!exerciseDropdown.contains(document.activeElement)) {
                            this.hideExerciseDropdown();
                        }
                    }, 150);
                });
            }
            ...
            // Parameter controls
            const controlBtns = document.querySelectorAll('.control-btn, .quick-btn');
            controlBtns.forEach(btn => {
                btn.addEventListener('click', (e) => {
                    e.preventDefault();
                    this.handleParameterControl(btn);
                });
            });
            
            // Input change handlers
            if (weightInput) {
                weightInput.addEventListener('input', () => this.updateButtonStates());
            }
            
            if (repsInput) {
                repsInput.addEventListener('input', () => this.updateButtonStates());
            }
        ...
        handleExerciseSearch(query) {
            ...
            this.populateExerciseDropdown(filteredExercises);
            this.showExerciseDropdown();
        },
        ...
        showExerciseDropdown() {
            ...
            if (dropdown) {
                ...
                if (dropdown.children.length === 0) {
                    this.populateExerciseDropdown(this.exercises);
                }
            }
        },
        ...
        populateExerciseDropdown(exercises) {
            ...
            if (exercises.length === 0) {
                dropdown.innerHTML = '<div class="exercise-option" style="text-align: center; color: var(--text-secondary);">Упражнения не найдены</div>';
                return;
            }
        ...
        handleParameterControl(button) {
            ...
            if (target === 'weight') {
                ...
                input = document.getElementById('reps-input');
            }
            ...
            input.value = target === 'weight' ? newValue.toFixed(1) : Math.round(newValue);
        ...
        updateButtonStates() {
            ...
            const hasExercise = exerciseInput?.value.trim() && this.selectedExercise;
            const hasWeight = parseFloat(weightInput?.value || 0) > 0;
            const hasReps = parseFloat(repsInput?.value || 0) > 0;
            
            const canAdd = hasExercise && (hasWeight || hasReps);
            
            if (addExerciseBtn) {
                addExerciseBtn.disabled = !canAdd;
            }
            
            if (addSupersetBtn) {
                addSupersetBtn.disabled = !canAdd;
            }
        },
        ...
        addSuperset() {
            ...
            const firstExercise = {
                ...
                sets: [{
                    ...
                    reps: parseFloat(document.getElementById('reps-input')?.value || 0)
                }]
            };
            ...
            if (addBtn) {
                addBtn.textContent = 'Завершить суперсет';
        ...
        validateForm() {
            ...
            if (!exerciseInput?.value.trim() || !this.selectedExercise) {
                Utils.showError('Выберите упражнение');
                return false;
            }
            ...
            if (weight <= 0 && reps <= 0) {
                Utils.showError('Укажите вес или количество повторений');
                return false;
            }
        ...
        resetForm() {
            ...
            if (repsInput) repsInput.value = '0';
    ...
    // Keep only workout detail modal
    Modal: {
        show(modalId) {
